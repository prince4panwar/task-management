  ...
  return (
    <aside
      className={`relative hidden sm:flex flex-col gap-2 align-center
        lg:p-2 p-1 h-[calc(100vh-70px)] mt-0.5 bg-slate-300
        transition-[width] duration-300 ease-in-out
        ${theme === "light" ? "light" : "dark"}
        ${showSidebar ? "w-[20%]" : "w-[5%]"}
      `}
    >
      <CircleChevronLeft
      ...
      {showSidebar && (
        <span className=" lg:text-lg text-sm">Welcome {user?.name}</span>
      )}
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            {showSidebar && <span>Create Tasks</span>}
          </button>
        </TooltipTrigger>
        {!showSidebar && (
          <TooltipContent side="right" align="center">
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            {showSidebar && <span>Tasks Summary</span>}
          </button>
        </TooltipTrigger>
        {!showSidebar && (
          <TooltipContent side="right" align="center">
